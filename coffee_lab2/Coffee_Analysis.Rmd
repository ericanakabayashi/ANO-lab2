...
```{r exclude oceania}
df <- subset(df, subregion != "Oceania") #clean invalid score
...
```{r model3}
v3<-lm(train$Total.Cup.Points ~ train$altitude + train$subregion + train$Tropical + train$subregion*train$Tropical )
plot(v3)
summary(v3)
```
```{r stargazer_}
